...
class Bittrex(Exchange):
    ...
    def get_orderbook(self, market, type=BOTH_ORDERBOOK):
        """
        ...
        }, options={'market': market, 'marketname': market, 'type': type}, protection=PROTECTION_PUB)
    ...
    def buy(self, currency, amount):
        market = "USDT-" + self.currencies[currency].identifier
        rate = self.get_orderbook(market, type="buy")["result"][0]
        return self.buy_limit(market, amount, rate)

    def sell(self, currency, amount):
        market = "USDT-" + self.currencies[currency].identifier
        rate = self.get_orderbook(market, type="sell")["result"][0]
        return self.sell_limit(market, amount, rate)
    ...
    def get_balance(self, currency):
        ...
        :param currency: Enum currency for the currency (ie. Currencies.bitcoin)
        :type currency: str
        ...
        }, options={'currency': currency_identifier, 'currencyname': currency_identifier}, protection=PROTECTION_PRV)
        if r["result"]:  # if our result is not none
            try:
    ...
    def transfer(self, currency, amount, address):
        ...
        return self._api_query(path_dict={
            API_V1_1: '/account/withdraw',
            API_V2_0: '/key/balance/withdrawcurrency'
        }, options={'currency': currency_identifier, 'quantity': amount, 'address': address},
            protection=PROTECTION_PRV)
